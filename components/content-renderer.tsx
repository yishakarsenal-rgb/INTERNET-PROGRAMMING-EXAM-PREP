...
/**
 * Renders markdown-like content string to styled React elements.
 * Supports: headings, bold, inline-code, fenced code blocks, tables, bullet lists, numbered lists.
 */
            ...
            <thead className="bg-muted">
              <tr>
                {headerCells.map((cell, j) => (
                  <th key={j} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">
                    {inlineFormat(cell)}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((row, ri) => (
                <tr key={ri} className="hover:bg-muted/30 transition-colors">
                  {row.map((cell, ci) => (
                    <td key={ci} className="px-4 py-2.5 text-xs text-foreground/90 leading-relaxed">
                      {inlineFormat(cell)}
                    </td>
                  ))}
                </tr>
    ...
    if (/^\d+\. /.test(line)) {
      ...
      elements.push(
        <ol key={i} className="my-3 space-y-1.5 ml-4 list-decimal list-outside">
          {items.map((item, j) => (
            <li key={j} className="text-sm text-foreground/90 leading-relaxed ml-4">
              {inlineFormat(item)}
            </li>
          ))}
        </ol>
