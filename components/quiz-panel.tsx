...
export default function QuizPanel({ questions, existingScore, onComplete }: Props) {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1">
          {questions.map((_, i) => (
            <button
              key={i}
              onClick={() => { setCurrentIndex(i); setSelectedIndex(answers[i]) }}
      ...
      <div className={`p-6 rounded-2xl border mb-6 text-center ${
        ...
        <div className="flex justify-center mb-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${
            isPerfect ? "bg-emerald-500" : isPassing ? "bg-yellow-500" : "bg-red-500"
          }`}>
            <Trophy className="w-8 h-8 text-white" />
          </div>
        </div>
        <p className="text-4xl font-bold text-foreground mb-1">{score}/{questions.length}</p>
        <p className={`text-lg font-medium mb-1 ${
          isPerfect ? "text-emerald-400" : isPassing ? "text-yellow-400" : "text-red-400"
        }`}>
          {isPerfect ? "Perfect Score!" : isPassing ? "Good job!" : "Keep practicing!"}
        </p>
        <p className="text-sm text-muted-foreground">{percent}% correct</p>
      </div>

      {/* Question review */}
      <h3 className="text-sm font-semibold text-foreground mb-3">Review Answers</h3>
